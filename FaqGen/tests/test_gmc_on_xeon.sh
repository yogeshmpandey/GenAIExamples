...
function validate_faqgen() {
    ...
    sed -i "s|namespace: faqgen|namespace: $APP_NAMESPACE|g"  ./faqgen_xeon.yaml
    kubectl apply -f ./faqgen_xeon.yaml

    # Wait until the router service is ready
    echo "Waiting for the faqgen router service to be ready..."
    wait_until_pod_ready "faqgen router" $APP_NAMESPACE "router-service"
    ...
    # send request to faqgen
    export CLIENT_POD=$(kubectl get pod -n $APP_NAMESPACE -l app=client-test -o jsonpath={.items..metadata.name})
    echo "$CLIENT_POD"
    accessUrl=$(kubectl get gmc -n $APP_NAMESPACE -o jsonpath="{.items[?(@.metadata.name=='faqgen')].status.accessUrl}")
    kubectl exec "$CLIENT_POD" -n $APP_NAMESPACE -- curl $accessUrl  -X POST  -d '{"query":"Text Embeddings Inference (TEI) is a toolkit for deploying and serving open source text embeddings and sequence classification models. TEI enables high-performance extraction for the most popular models, including FlagEmbedding, Ember, GTE and E5."}'  -H 'Content-Type: application/json' > $LOG_PATH/gmc_faqgen.log
    exit_code=$?
    if [ $exit_code -ne 0 ]; then
        echo "faqgen failed, please check the logs in ${LOG_PATH}!"
        exit 1
    fi

    cat $LOG_PATH/gmc_faqgen.log
    echo "Checking response results, make sure the output is reasonable. "
    local status=false
    if [[ -f $LOG_PATH/gmc_faqgen.log ]] && \
    [[ $(grep -c "[DONE]" $LOG_PATH/gmc_faqgen.log) != 0 ]]; then
        status=true
    fi
    if [ $status == false ]; then
        if [[ -f $LOG_PATH/gmc_faqgen.log ]]; then
            cat $LOG_PATH/gmc_faqgen.log
        fi
